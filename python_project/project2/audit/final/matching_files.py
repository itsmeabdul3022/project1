...
def process_files(file_a_path, file_b_path, matching_output_path, unique_output_path):
    ...
    # Process file_a and store matching lines in a set to ensure uniqueness
            ...
            for line in f_a:
                ...
                if match:
                    ...
                    if other_data in file_b_data:
                        dotted_number_b = file_b_data[other_data]
                       # new_line = f'description : "{new_dotted_number} {match.group(1).strip()}"\n'
                        new_line = f'description : "{dotted_number_a} {match.group(2).strip()}" {dotted_number_b} \n'
    ...
    # Convert the set to a list, sort, and write to the output file
    # matching_lines = sorted(list(matching_lines_set), key=sort_key_by_dotted_number)
    # try:
    #    with open(matching_output_path, 'w') as matching_out:
    #         matching_out.writelines(matching_lines)
    # except IOError:
    #   print(f"Error: Could not write to '{matching_output_path}'.")
process_files('CIS_IBM_AIX_7_v1.0.0_L1_CUSTOM_TEST3142025.audit.rules.out', 'cis_rules.txt', 'CIS_IBM_AIX_7_v1.0.0_L1_CUSTOM_TEST3142025.audit.matching2.out', 'CIS_IBM_AIX_7_v1.0.0_L1_CUSTOM_TEST3142025.audit.unique2.out')